...
A3S_C_VALUES = {  # include quotes explicitly for strings.
    'STRING_CONFIG_H_AUTHOR': (
        '"(marlininfo, part of maniforge by Jake Gustafson, Telli Mantelli, Kris Waclawski,'
...
BLTOUCH_C_COMMENTS = {
    'AUTO_BED_LEVELING_BILINEAR': (
        ...
        " but UBL supersedes it and MESH_BED_LEVELING"
    ),
...
class MarlinInfo:
    ...
    def __init__(self, path, repo=None):
        ...
        for relpath in MarlinInfo.TRANSFER_RELPATHS:
            ...
            if os.path.isfile(file_path):
                ...
                logger.info('* there is no "{}"'.format(file_path))
    ...
    def load_repo(self):
        ...
        try:
            ...
            if not repo.bare:
                ...
                logger.error('Warning: diff features will not be available '
    ...
    def backup_config(self):
        ...
        for relpath in MarlinInfo.TRANSFER_RELPATHS:
            ...
            if not os.path.isfile(bak):
                ...
                '''
                shutil.copy(old, bak)
                logger.error('- backed up "{}"'.format(bak))
            else:
                logger.error('- kept existing "{}"'.format(bak))
        return True
    ...
    def save_changes(self):
        ...
        for relpath, fi in self.file_metas.items():
            logger.info('saving "{}"'.format(fi.full_path()))
            this_count = fi.save_changes()
            logger.error('* saved {} change(s) to "{}"'
    ...
    def set_c(self, name, value, comments=None):
        ...
        if len(lines) > 0:
            # logger.debug("{}: #define {} {}".format(fi.full_path(), name, value))
            for line in lines:
                logger.debug("{}: `{}`".format(fi.full_path(), line))
        else:
            logger.debug("(skipped) {}: {} to {}".format(fi.full_path(), name, value))
...
def which(cmd):
    ...
    if paths_str is None:
        logger.info("Warning: There is no PATH variable, so returning {}"
              "".format(cmd))
        return cmd
    ...
    for path in paths:
        ...
        if os.path.isfile(tryPath):
            ...
            logger.info("There is no {}".format(tryPath))
    return None
...
def get_repo(path):
    ...
    if (repo is not None) and repo.bare:
        logger.error("The repo is bare.")
        repo = None
    if repo is None:
        # logger.error('  branch: "{}"'.format())
        sys.stderr.write(
...
def main():
    try:
        ...
        logger.error(str(ex))
        logger.error("You must run this from a Marlin repo"
    ...
    for argI in range(1, len(sys.argv)):
        ...
        if key is not None:
            ...
            if arg == "--verbose":
                ...
                usage()
                logger.error("The argument is incorrect: {}".format(arg))
                return 1
        else:
            if dst_repo_path is None:
                ...
                usage()
                logger.error('You specified an extra argument: "{}"'
                      ''.format(arg))
                return 1
    ...
    logger.error("Stats:")
    # logger.error(dir(repo))
    logger.error('  active_branch="{}"'.format(repo.active_branch))
    # logger.error('  tag="{}"'.format(repo.tag("refs/tags/"+versionTagStr)))
    # logger.error('  tree="{}"'.format(repo.tree()))  # hash
    # logger.error('  head="{}"'.format(repo.head))  # usually "HEAD"
    # logger.error('  remote="{}"'.format(repo.remote()))  # usually "origin"
    logger.error('  commit="{}"'.format(repo.commit()))
    changed = list(repo.index.diff(None))
    if len(changed) > 0:
        logger.error("  changed:")
        for item in changed:
            logger.error('  - "{}"'.format(item.a_path))
    else:
        logger.error("  changed: None")
    ...
    if in_place:
        try:
            ...
            raise
        logger.error('Modifying in place (setting source to backup: "{}")'
              ''.format(srcMarlin.m_path))
    # else it was already set to os.getcwd()

    logger.error('dst_repo_path="{}"'.format(dst_repo_path))
    logger.error('source Configuration.h, _adv versions: {}'
          ''.format(srcMarlin.get_confs_versions()))
    logger.error('destination Configuration.h, _adv versions: {}'
          ''.format(dstMarlin.get_confs_versions()))
    if dstMarlin.get_confs_versions() != srcMarlin.get_confs_versions():
        logger.error("Error: The configuration versions are incompatible")
        logger.error(" ({} != {}) First try:".format(
            dstMarlin.get_confs_versions(),
            srcMarlin.get_confs_versions(),
        ))
        logger.error('  meld "{}" "{}"'.format(srcMarlin.mm_path, dstMarlin.mm_path))
        return 1
    ...
    if len(results) > 0:
        for result in results:
            logger.error('* wrote "{}"'.format(result))
    else:
        raise FileNotFoundError(
    ...
    if "A3S" in this_marlin_name:
        if machine is None:
            ...
            logger.error('Warning: You set machine to {},'
    ...
    # set_verbosity(2)
    logger.debug("* setting Configuration.h values for {}".format(machine))
    thisMarlin.set_multiple_c(
        MACHINES_C_VALUES[machine],
        comment_d=MACHINES_C_COMMENTS.get(machine),
    )
    # set_verbosity(previous_verbosity)
    logger.debug("* setting Configuration_adv.h values for {}".format(machine))
    thisMarlin.set_multiple_c_a(
        ...
        if zmax_answer is not True:
            while True:
                ...
                if zmax_answer in ["y", "yes"]:
                    ...
                    logger.error("Specify y/yes or n/no.")
    ...
    if driver_type is None:
        logger.error("Please specify --driver-type such as one of {} and try again."
              "".format(driver_types))
        return 0
    if driver_type is not None:
